...
class MovieScraper:
    ...
    def __init__ (self,url: str=THEMOVIEDB_API_URL, online=True):
        self.apiUrl = url
        self.online = online
    ...
    def getConfiguration (self):
        try:
            status, result = self.__apicall__(CONFIG_ENDPOINT)
            if (status == 200):
                self.imagesUrl = result["images"]["secure_base_url"]+IMAGE_SIZE
        except Exception as e:
            logging.error(e)
    ...
    def getGenres (self,type:str="M"):
        try:
            if (type == "M"):
                status, result = self.__apicall__(GENRES_MOVIES_ENDPOINT)
            else: 
                status, result = self.__apicall__(GENRES_TV_ENDPOINT)
    ...
    def discoverMovies (self,pretty:bool=False):
        try:
            ...
            status, result = self.__apicall__(DISCOVER_MOVIE_ENDPOINT,params=parameters)
            if (status == 200):
                if (pretty):
    ...
    def getMovieById (self,id:str,pretty:bool=False):
        try:
            ...
            url = parse.urljoin(MOVIE_ENDPOINT,str(id))
    ...
    def getMovieCastCrew (self,id:str,pretty:bool=False):
        try:
            parameters = {"language":"es"}
            url = parse.urljoin(MOVIE_ENDPOINT,str(id)+"/credits")
            status, result = self.__apicall__(url,params=parameters)
            if (status == 200):
                ...
                if (pretty):
                    ...
                    indent_spaces = 0
                cast = df_cast.loc[df_cast['order']<3].filter(["name","character","order"]).to_json(orient="records",indent=indent_spaces)
    ...
    def getMovieByIMDBId (self,imdbId:str,pretty:bool=False):
        try:
            ...
            url = parse.urljoin(FIND_EXTERNAL_ENDPOINT,imdbId)
            status, result = self.__apicall__(url,params=parameters)
            if (status == 200):
    ...
    def getMovieImageUrl (self,id:str):
        try:
            ...
            url = parse.urljoin(MOVIE_ENDPOINT,str(id)+MOVIE_IMAGE_ENDPOINT)
            status, result = self.__apicall__(url,params=parameters)
            backdrop_url = ""
            poster_url = ""
            if (status == 200):
